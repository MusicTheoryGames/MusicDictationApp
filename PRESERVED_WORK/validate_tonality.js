...
const SCALE_DEFINITIONS = {
    'C_major': {
        ...
        accidentals: [], // C major should only use natural notes
        description: 'C Major (natural notes only)'
    },
    'A_minor_comprehensive': {
        natural: ['a', 'b', 'c', 'd', 'e', 'f', 'g'],
        accidentals: ['f#', 'g#', 'c#', 'd#', 'a#'], // All forms of minor plus chromatic embellishments
        description: 'A Minor (comprehensive: natural, harmonic, melodic, and chromatic)'
    }
};
...
function detectTonality(option) {
    ...
    // Try each tonality and score based on valid notes vs invalid notes
    for (const tonality of tonalities) {
        const result = validateOptionTonality(option, tonality, 0, 0); // Use dummy numbers for detection
        const totalNotes = result.allUsedNotes.length;
        const invalidNotes = result.allInvalidNotes.length;
        const validNotes = totalNotes - invalidNotes;

        // Score = percentage of valid notes (0-100)
        scores[tonality] = totalNotes > 0 ? (validNotes / totalNotes) * 100 : 0;
    }

    // Return the tonality with the highest score (most notes fitting)
    const bestTonality = Object.keys(scores).reduce((a, b) => scores[a] > scores[b] ? a : b);

    // If we have a perfect or near-perfect match (90%+), use it
    // Otherwise, prefer the order: harmonic minor, melodic minor, natural minor for A-minor pieces
    if (scores[bestTonality] >= 90) {
        return bestTonality;
    }

    // For A-minor pieces, prefer more complex forms if they fit well
    const minorTonalities = ['A_minor_harmonic', 'A_minor_melodic', 'A_minor_natural'];
    for (const tonality of minorTonalities) {
        if (scores[tonality] >= 80) {
            return tonality;
        }
    }
